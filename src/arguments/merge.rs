...
pub fn merge(name: &String) -> Result<(), io::Error> {
    
    let target_branch = Branch::get_branch(name.to_owned())?;
    let current_branch = Branch::get_current_branch();
    
    let current_commit = Commit::get_commit_from_file(current_branch.get_head().to_owned());
    let target_commmit = Commit::get_commit_from_file(target_branch.get_head().to_owned());
