...
impl Commit {
    ...
    pub fn get_commit_from_file(hash: String) -> Result<Commit, io::Error>{
        ...
        Ok(Commit::new(String::from(tree), String::from(parent), description))
    }
    
    pub fn create_commit_tree(branch: Branch) -> Result<Option<Node>, io::Error>{
    ...
    pub fn get_commit_list(branch_name: String) -> Result<Vec<Commit>, io::Error>{
        ...
        for line in reader.lines() {
            ...
            Commit::get_commit_from_file(content)?;
    ...
    pub fn commit_exist(hash: &String) -> Result<bool,DitError> {
        let branch = Branch::get_current_branch()?;
        let commits = Self::get_commit_list(branch.get_name().to_string()).map_err(DitError::IoError)?;
    ...
    pub fn display_commit_tree() -> Result<(), DitError>{
        let root = Commit::create_commit_tree(Branch::get_current_branch()?).map_err(DitError::IoError)?;
        match root {
